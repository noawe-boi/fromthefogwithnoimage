...
$say The value $(value) is in the $(index) position in an array which has a length of $(length) and you are seeing this message from this command $(command) with info from $(target) $(path).